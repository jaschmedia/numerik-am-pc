function [x, N] = adaptDivDiff(f, a, b, n)
    ...
    x = [a; b];
    N = divDiff(x, f(x)); % Berechne die dividierten Differenzen zu x und f(x)
    xi = [0.5*(a+b)];
    fxi = f(xi);
    ...
    for ii=2:n
        ...
        [x, N] = addDivDiff(x,N,xi(jj),fxi(jj));
                     % erweitere die dividierten Differenzen N zu den Knoten x
                     % mit dem Knoten xi(jj) und dessen Funktionswert fxi(jj)
